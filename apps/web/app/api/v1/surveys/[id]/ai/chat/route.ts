...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const result = await provider.chatAssistant({ instruction: parsed.data.instruction, structure });
  ...
  await prisma.aiGeneration.create({
    data: {
      ...
      promptSummary: parsed.data.instruction,
  ...
  return NextResponse.json({ survey: updated, reply: result.reply });
}
