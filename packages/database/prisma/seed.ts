import { PrismaClient, type OrgRole, type QuestionType, type SurveyStatus } from "@prisma/client";
...
async function main() {
  console.log("Seeding SurveyMasterAI demo data...");
  ...
  console.log(`  - ${SURVEY_TEMPLATES.length} survey templates`);
  ...
  console.log(`  - organization "${org.name}" with ${demoUsers.length} members`);
  ...
  // --- Demo surveys built from templates -----------------------------------
  ...
  for (const spec of surveySpecs) {
    ...
    if (spec.responseCount > 0) {
      await generateResponses(survey, spec.responseCount);
    }
  }

  console.log(`  - ${surveySpecs.length} demo surveys with responses`);
...
async function generateResponses(survey: DemoSurveyWithQuestions, count: number) {
